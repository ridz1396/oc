...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'stock_status_schema_url' => $query->row['stock_status_schema_url'],
	...
	public function getProducts($data = array()) {
		...
		if(!empty($data['filter_price'])) {
			...
			$sql .= " AND p.price BETWEEN " . (int)$price[0] . " AND " . (int)$price[1];
		}

		if(!empty($data['filter_status'])) {
			$implode = array();

			$filters = explode(',', $data['filter_status']);

			foreach ($filters as $filter_id) {
				$implode[] = (int)$filter_id;
			}

			if(in_array(7, $filters) && count($filters) === 1) {
				$sql .= " AND p.quantity > 0";
			} else if(in_array(7, $filters)) {
				$key = array_search(7, $data);
				if ($key !== false) {
					unset($data[$key]);
				}
				$sql .= " AND p.stock_status_id IN (" . implode(',', $implode) . ")";
			} else {
				$sql .= " AND p.stock_status_id IN (" . implode(',', $implode) . ") AND p.quantity < 1";
			}
		}
